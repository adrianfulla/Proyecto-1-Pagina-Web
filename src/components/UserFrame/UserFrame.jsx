...
const UserFrame = ({UsersOnline, UsersInGame}) => {
    const [UsersOn, setUsersOn] = useState(UsersOnline)
    const [UsersIn, setUsersIn] = useState(UsersInGame)

    useEffect(() => {
        const interval = setInterval(() => {
            setUsersOn((UsersOn) => (parseInt(UsersOn) + Math.floor(Math.random() * 200 + 1)))
            setUsersIn((UsersIn) => (parseInt(UsersIn) + Math.floor(Math.random() * 200 + 1))) }, 1000)
    ...
    return (
        <section className="UserFrameWrapper">
              <div className="steam_section">
                <div className="row">
                  <div className="col_2">
                    <img className="steam_logo_about" src="https://cdn.akamai.steamstatic.com/valvesoftware/images/about/steam_logo.png" />
                  </div>
